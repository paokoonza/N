...
cl = LINETCR.LINE()
cl.login(token="EpwCDINsHztjfbjPNeM5.FsWACvo8kRbxJig+CjYizq.KnjoYlHXX47FiZdVItgysvCOkeuVvKNdHXS/iNwBRY8=")
cl.loginResult()
ki=ki2=ki3=ki4=cl
...
#-----------------------------------------------
        if op.type == 17:
            ...
            cb = Message()
            cb.to = op.param1
            cb.text = cl.getContact(op.param2).displayName + "[🙏สวัดดีจ้า ยินดีต้อนรับน๊ะ🙏\n💋แนะนำตัวเองด้วยน๊ะ💋\n\n[By.มินทีมทดลองบอท]]"
            cl.sendMessage(cb)

        if op.type == 15:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1, "มึงจะรีบไปไหน\nมาอยู่เปย์สติ๊กเกอร์ก่อนกูก่อน\n\n[By.มินทีมทดลองบอท]")
            print "MemberLeft"
#----------------------------------------------- 
#------------------------------------------------------------------------------------
